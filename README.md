...
> **👉 https://interactive-voronoi.vercel.app/** — 브라우저에서 바로 실행 (60fps, WebGL)
...
| 영역 | 사용 기술 |
|---|---|
| **렌더링** | WebGL 1.0 / 2.0 Fragment Shader (GLSL) |
| **거리 계산** | 픽셀별 nearest-point search (셰이더 루프) |
...
- [ ] **v0.1** — 단일 HTML MVP: WebGL 보로노이 + 파스텔 색상 + 정적 점
- [ ] **v0.2** — 마우스 disturbance + spring/damping 보간
- [ ] **v0.3** — 자동 부유 (perlin noise drift)
- [ ] **v0.4** — Vercel 배포 + 라이브 데모
- [ ] **v0.5** — 사운드 통합 (healing ambient tone)
- [ ] **v0.6** — Record & replay (MP4 / WebM 캡처)
- [ ] **v1.0** — Playwright E2E 테스트 (Canvas 픽셀 분산 검증)
